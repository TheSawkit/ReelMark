...
const BADGE_CLASSES = "flex items-center gap-2 bg-glass-bg-hover backdrop-blur-2xl backdrop-saturate-150 px-4 py-2 rounded-full border border-glass-border-hover shadow-card-sm"

export function CommunityRatingBadge({
    rating, isWatched, mediaId, mediaType, mediaTitle, posterPath, initialReview,
    ...
    const ariaLabel = initialReview
        ? t.movie.editReview
        : mediaType === "tv" ? t.movie.rateTvShow : t.movie.rateMovie
        ...
        <>
            ...
            <span className="font-semibold text-text tabular-nums">
                ...
                {rating && rating.count > 0 && (
                    <span className="text-muted text-xs font-normal ml-1.5">({rating.count})</span>
                )}
            </span>
        </>
    )
