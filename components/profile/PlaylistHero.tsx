...
export function PlaylistHero({ playlist, ownerUsername, ownerAvatarUrl }: PlaylistHeroProps) {
    const { t } = useTranslation()
    const items = playlist.items ?? []
    const previewItems = items.slice(0, 5)
    const backgroundPoster = items[0]?.poster_path
    const handleShare = async () => {
        await navigator.clipboard.writeText(`${BASE_URL}/playlist/${playlist.id}`)
        ...
        <section className="relative -mt-16 overflow-hidden min-h-[20vh] md:min-h-[50vh] flex flex-col justify-end">
            ...
            >
                ...
                {previewItems.length > 0 && (
                    <div className="group flex items-end shrink-0 max-lg:hidden" aria-hidden>
