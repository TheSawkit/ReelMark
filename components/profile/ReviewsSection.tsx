                        ...
                        {review.media_type !== 'episode' ? (
                            <Link href={`/${review.media_type}/${review.media_id}`} className="shrink-0">
                                <div className="relative w-12 aspect-2/3 rounded-poster overflow-hidden bg-surface-2">
                                    {review.poster_path ? (
                                        <Image src={getImageUrl(review.poster_path, 'w92')} alt={review.media_title} fill sizes="48px" className="object-cover" />
                                    ) : (
                        ...
                        <div className="flex-1 min-w-0">
                            {review.media_type !== 'episode' ? (
                                <Link href={`/${review.media_type}/${review.media_id}`}>
                                    <p className="font-medium text-sm text-text hover:text-primary transition-colors truncate">{review.media_title}</p>
                                </Link>
                            ) : (
                                <p className="font-medium text-sm text-text truncate">{review.media_title}</p>
                            )}
                            {review.rating != null && (
                                <div className="mt-1">
