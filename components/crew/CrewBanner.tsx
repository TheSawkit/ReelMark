...
export function CrewBanner({ crew }: CrewBannerProps) {
    ...
    function getJobLabel() {
        ...
        if (!dept) return '';
        const key = DEPARTMENT_KEY_MAP[dept] || dept.toLowerCase().replace(/[^a-z0-9]+/gi, '');

        const jobTitles = (t.movie as any)?.jobTitles;
        if (jobTitles && jobTitles[key]) {
            const isFemale = crew.gender === 1;
            const titleObj = jobTitles[key];
            ...
            <div className="relative z-10 container mx-auto px-6 lg:px-12 py-12 md:py-16">
                <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center md:items-start">
                    ...
                    <div className="flex-1 text-center md:text-left">
                        ...
                        <p className="text-lg text-muted mb-6">{getJobLabel()}</p>
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 md:gap-4">
                            ...
                            {crew.deathday && (
                                <div className="flex items-center gap-2 glass-surface px-3 py-1.5 rounded-full">
                                    ...
                                    <span className="text-sm text-text">† {formatDate(crew.deathday, locale)}</span>
                                </div>
                            )}
                            ...
                            {crew.place_of_birth && (
                                <div className="flex items-center gap-2 glass-surface px-3 py-1.5 rounded-full">
                                    ...
                                    <span className="text-sm text-text">{crew.place_of_birth}</span>
                                </div>
                            )}

                            <div className="flex items-center gap-2 glass-surface px-3 py-1.5 rounded-full">
                                <Star className="h-4 w-4 fill-gold text-gold" />
                                <span className="text-sm font-semibold text-text">{(crew.popularity || 0).toFixed(0)}</span>
                            </div>
                        </div>
                    </div>
                </div>
