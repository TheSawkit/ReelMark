...
describe('canViewWithVisibility', () => {
    it('owner can always view', () => {
        expect(
            canViewWithVisibility('private', { isOwn: true, isFriend: false })
        ).toBe(true);
        expect(
            canViewWithVisibility('friends', { isOwn: true, isFriend: false })
        ).toBe(true);
        expect(
            canViewWithVisibility('public', { isOwn: true, isFriend: false })
        ).toBe(true);
    });

    it('public is visible to everyone', () => {
        expect(
            canViewWithVisibility('public', { isOwn: false, isFriend: false })
        ).toBe(true);
        expect(
            canViewWithVisibility('public', { isOwn: false, isFriend: true })
        ).toBe(true);
    });

    it('friends-only is visible to friends, not strangers', () => {
        expect(
            canViewWithVisibility('friends', { isOwn: false, isFriend: true })
        ).toBe(true);
        expect(
            canViewWithVisibility('friends', { isOwn: false, isFriend: false })
        ).toBe(false);
    });

    it('private is never visible to non-owners', () => {
        expect(
            canViewWithVisibility('private', { isOwn: false, isFriend: false })
        ).toBe(false);
        expect(
            canViewWithVisibility('private', { isOwn: false, isFriend: true })
        ).toBe(false);
    });
});
