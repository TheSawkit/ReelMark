...
import { buildPageMetadata, buildMediaMetadata, BASE_URL } from "@/lib/metadata"
...
describe("buildPageMetadata", () => {
    ...
    it("adds robots noindex when isPrivate is true", () => {
        const meta = buildPageMetadata("Dashboard", "Private", { isPrivate: true })
    ...
    it("sets alternates.canonical when provided", () => {
        const meta = buildPageMetadata("Page", "Desc", { canonical: "https://reelmark.app/page" })
...
describe("buildMediaMetadata", () => {
    ...
    it("sets title, description, and canonical", () => {
        ...
        expect(meta.description).toBe("A team of explorers travel through a wormhole.")
        expect(meta.alternates?.canonical).toBe("https://reelmark.app/movie/157336")
    })
    ...
    it("sets openGraph type from ogType parameter", () => {
        ...
        expect((tvMeta.openGraph as { type?: string })?.type).toBe("video.tv_show")
    })

    it("populates images when backdropPath is provided", () => {
        const meta = buildMediaMetadata(base)
        const ogImages = meta.openGraph?.images as Array<{ url: string; width: number; height: number }>
