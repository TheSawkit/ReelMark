...
import type { WatchlistEntry, ActorMovieCredit, ActorTvCredit } from "@/types/tmdb"
...
describe("watchlistEntryToMediaItem", () => {
    ...
    it("works with tv media_type", () => {
        const tvEntry: WatchlistEntry = { ...baseWatchlistEntry, media_type: "tv", media_title: "Breaking Bad" }
...
describe("movieCreditToMediaItem", () => {
    ...
    it("maps all fields correctly", () => {
        ...
        expect(item.title).toBe("Fight Club")
        expect(item.original_title).toBe("Fight Club")
        expect(item.overview).toBe("An insomniac office worker forms an underground club.")
        expect(item.poster_path).toBe("/poster.jpg")
    ...
    it("sets character to undefined when empty string", () => {
        const item = movieCreditToMediaItem({ ...baseMovieCredit, character: "" })
