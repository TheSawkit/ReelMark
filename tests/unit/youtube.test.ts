...
const yt = (key: string): Video => ({ key, site: "YouTube", name: `Video ${key}` })
const vimeo = (key: string): Video => ({ key, site: "Vimeo", name: `Vimeo ${key}` })
...
describe("filterAvailableVideos", () => {
    ...
    it("excludes YouTube video when fetch throws", async () => {
        globalThis.fetch = (() => { throw new Error("Network error") }) as typeof fetch
        try {
            const result = await filterAvailableVideos([yt("abc123")])
            expect(result).toHaveLength(0)
        } finally {
