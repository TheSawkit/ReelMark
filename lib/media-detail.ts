...
export async function buildMediaDetailMetadata(type: MediaType, id: number): Promise<Metadata> {
    const t = await getTranslations()
    const defaultDesc = type === "movie" ? t.metadata.defaultMovieDescription : t.metadata.defaultTvDescription

    try {
        if (type === "movie") {
            const details = await getMovieDetails(id)
            const description = details.overview || t.metadata.watchMovieOn.replace("${title}", details.title)
            ...
            const description = details.overview || t.metadata.watchShowOn.replace("${title}", details.name)
            return buildMediaMetadata({
                title: details.name,
                description,
                backdropPath: details.backdrop_path,
