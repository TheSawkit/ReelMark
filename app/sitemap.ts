...
import { getPopularMovies, getTopRatedMovies, getPopularTvShows, getTopRatedTvShows } from "@/lib/tmdb"
...
const STATIC_ROUTES: MetadataRoute.Sitemap = [
    ...
    { url: `${BASE_URL}/explorer`, lastModified: new Date(), changeFrequency: "daily", priority: 0.8 },
]

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    const [moviesP1, moviesP2, topMoviesP1, tvP1, tvP2, topTvP1] = await Promise.allSettled([
        getPopularMovies(1),
        getPopularMovies(2),
        getTopRatedMovies(1),
        getPopularTvShows(1),
    ...
    const movieRoutes: MetadataRoute.Sitemap = [...new Map(movies.map(m => [m.id, m])).values()].map(m => ({
    ...
    const tvRoutes: MetadataRoute.Sitemap = [...new Map(shows.map(s => [s.id, s])).values()].map(s => ({
