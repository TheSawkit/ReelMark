...
async function fetchTmdbForType(
  ...
  try {
    ...
    const data = await fetchTMDB<{ name: string; poster_path: string | null }>(
      `/tv/${id}`,
      {},
      86400
    );
...
export async function importBatch(
    ...
    items.map(async (item): Promise<Resolved> => {
      try {
        ...
        if (item.imdbId && /^tt\d+$/.test(item.imdbId)) {
          ...
          if (found)
            return resolve(found.id, mediaType, found.title, found.poster_path);
        }

        if (item.tvdbId && Number.isInteger(item.tvdbId) && item.tvdbId > 0) {
          const found = await findByTvdbId(item.tvdbId);
          if (found)
            return resolve(found.id, 'tv', found.title, found.poster_path);
        }
  ...
  const errors = await Promise.all([
    watchedRows.length > 0
      ? supabase
          .from('watchlist')
          .upsert(watchedRows, { onConflict: 'user_id,media_id,media_type' })
          .then((r) => r.error)
